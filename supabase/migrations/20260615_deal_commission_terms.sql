...
alter table public.files add column if not exists brokerage_split_pct numeric;     -- brokerage/company cut % (default 20; 50 for company leads)
alter table public.files add column if not exists team_split_pct numeric;          -- team lead's % of the brokerage split (default 10)
