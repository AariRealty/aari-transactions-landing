...
// GATE (July): "Coming up" deadlines withheld until effective_date_confirmed_at is set.
// Unified style (July): plain bordered cards (no black header), sans font, matches the rest.
// v15 (July 24): estoppel gated on HOA — no phantom estoppel deadline on no-HOA files.
...
const TASK_LABELS: Record<string, string> = {
  ...
  "app_satisfactory_confirm": "At/above value — confirm with lender appraisal is satisfactory",
  ...
  "app_low_addendum": "Below value — if price reduction agreed, confirm executed addendum",
  ...
  "ctc_title_exceptions_notify": "Title exceptions — notify agents and parties IMMEDIATELY",
  ...
  "cl_funds_recorded": "Confirm all closing funds collected and deed recorded",
  "cl_keys": "Confirm keys delivered to buyer",
  "cl_personal_property": "Confirm all personal property per contract on premises",
  "cl_docs_filed": "Confirm closing documents received and filed",
  "cl_congrats_agent": "Send closing congratulations email to agent",
  "cl_confirm_parties": "Send closing confirmation to all parties",
  "cl_google_review": "Request Google review from agent",
  "cl_closed_stamp": "Closed date stamped on file",
  "cl_review_24h": "24-hour post-closing review request",
...
function flBusinessDay(d: Date): Date { const x = new Date(d.getTime()); for(let i=0;i<10;i++){ const day = x.getUTCDay(); if(day!==0 && day!==6 && !FED_HOLIDAYS.has(ymd(x))) return x; x.setUTCDate(x.getUTCDate()+1); } return x; }
...
function fileDeadlines(file: any){
  ...
  // Estoppel is an HOA/condo-only deadline. Never surface it on a file with no
  // association, or a no-HOA agent sees a phantom "Estoppel letter deadline".
  ...
  for(const def of DEADLINE_DEFS){
    ...
    if(ov[def.key]){ const od = parseDate(ov[def.key]); if(od){ out.push({ label:def.label, date:od }); continue; } }
...
function nyParts(): { hour: number; dow: number } { const now = new Date(); const fmt = new Intl.DateTimeFormat("en-US", { timeZone:"America/New_York", hour:"numeric", hour12:false, weekday:"short" }); const parts = fmt.formatToParts(now); const hour = parseInt(parts.find(p=>p.type==="hour")?.value ?? "0", 10); const dowMap: Record<string, number> = { Sun:0, Mon:1, Tue:2, Wed:3, Thu:4, Fri:5, Sat:6 }; const dow = dowMap[parts.find(p=>p.type==="weekday")?.value ?? "Fri"] ?? 5; return { hour, dow }; }
function lastMondayEt(): Date { const et = new Date(new Date().toLocaleString("en-US", { timeZone:"America/New_York" })); const back = (et.getDay()+6)%7; et.setDate(et.getDate()-back); et.setHours(0,0,0,0); return et; }
const esc = (s: string) => (s || "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
const fmtD = (d: Date) => d.toLocaleDateString("en-US", { month:"short", day:"numeric", timeZone:"UTC" });
// deno-lint-ignore no-explicit-any
function stageOf(f: any): string { const s = (f.transaction_stage || "").toLowerCase(); if(STAGE_LABELS[s]) return s; if(f.status==="closed"||f.status==="archived") return "closed"; if(f.assigned_tc_id && !f.tc_accepted_at) return "waiting_for_tc"; return "new"; }

Deno.serve(async (req) => {
  let force = false;
  try { const b = await req.json(); force = !!b?.force; } catch { /* cron sends {} */ }
  const { hour, dow } = nyParts();
  if(!force && (dow !== 5 || hour !== 8)) return json({ ok:true, skipped:"outside_window", ny_hour:hour, ny_dow:dow });

  const { data: agents, error: agErr } = await supabaseAdmin.from("agents").select("id, first_name, email, weekly_digest_opt_in, role").eq("weekly_digest_opt_in", true).eq("role", "agent").not("email", "is", null);
  ...
  for(const agent of agents ?? []){
    try {
      const { data: files } = await supabaseAdmin.from("files").select("id, property_address, transaction_stage, status, tc_accepted_at, assigned_tc_id, closing_date, effective_date, effective_date_confirmed_at, deadline_overrides, logistics, stage_tasks, friday_summary_sent_at").eq("agent_id", agent.id).not("status", "in", '("closed","cancelled","archived")');
      const active = (files ?? []).filter(f => !f.friday_summary_sent_at || new Date(f.friday_summary_sent_at) < cutoff);
      ...
      if(tcId){ const { data } = await supabaseAdmin.from("agents").select("first_name, last_name, email, phone").eq("id", tcId).single(); tc = data ?? null; }
      ...
      const blocks = active.map(f => {
        ...
        let dtc = "";
        if(f.closing_date){ const cd = new Date(f.closing_date + "T00:00:00Z"); const days = Math.round((cd.getTime()-today.getTime())/86400000); dtc = days<=7 ? "Closing this week" : days + " days to close"; }
        const dt = (f.stage_tasks && typeof f.stage_tasks === "object") ? f.stage_tasks : {};
        const done = Object.keys(dt).filter(k => dt[k]?.at && new Date(dt[k].at) >= weekStart && TASK_LABELS[k]).map(k => TASK_LABELS[k]);
        const doneHtml = done.length ? '<ul style="margin:6px 0 0;padding-left:18px">' + done.map(l => '<li style="margin:4px 0">' + esc(l) + "</li>").join("") + "</ul>" : '<p style="margin:6px 0 0;color:#6b6b6b">Your TC is working behind the scenes on your file.</p>';
        const effConfirmed = !!f.effective_date_confirmed_at;
        const upcoming = !effConfirmed ? "" : fileDeadlines(f).filter(d => d.date.getTime() >= today.getTime()).sort((a,b)=>a.date.getTime()-b.date.getTime()).slice(0,2).map(d => { const days = Math.round((d.date.getTime()-today.getTime())/86400000); const urgent = days<=3; return '<li style="margin:4px 0;' + (urgent?"color:#9a3a2c;font-weight:600":"") + '">' + esc(d.label) + " " + fmtD(d.date) + (urgent?" · urgent":"") + "</li>"; }).join("");
        const cap = 'font-size:10px;letter-spacing:0.5px;color:#8a857c;text-transform:uppercase;margin-top:14px';
        const comingUp = upcoming ? '<div style="' + cap + '">Coming up</div><ul style="margin:6px 0 0;padding-left:18px">' + upcoming + "</ul>" : (!effConfirmed ? '<div style="' + cap + '">Coming up</div><p style="margin:6px 0 0;color:#6b6b6b">Confirm your effective date and your full deadline schedule appears here.</p>' : "");
        return '<div style="border:1px solid #e8e4db;border-radius:10px;margin:0 0 18px;padding:16px">' +
          '<div style="font-size:14px;font-weight:700">' + esc(f.property_address || "Your file") + '</div>' +
          '<div style="font-size:12px;color:#8a857c;margin-top:2px">' + esc(stage) + (dtc ? " · " + esc(dtc) : "") + '</div>' +
          '<div style="' + cap + ';margin-top:14px">Completed this week</div>' + doneHtml + comingUp + '</div>';
      }).join("");

      const subject = active.length === 1 ? "Your weekly update · " + (active[0].property_address || "your file") : "Your weekly update · " + active.length + " active files";
      const html = '<div style="font-family:-apple-system,BlinkMacSystemFont,Arial,sans-serif;max-width:560px;margin:0 auto;color:#0f0f0f">' +
        '<p style="font-size:14px;line-height:1.55;margin:0 0 16px">Hi ' + esc(agent.first_name || "there") + ", here is where everything stands this week:</p>" + blocks +
        '<p style="font-size:12.5px;color:#6b6b6b;line-height:1.55;margin:0 0 6px">Questions? Reply to this email or message your TC directly.</p>' +
        '<p style="font-size:12.5px;color:#6b6b6b;line-height:1.55;margin:0 0 14px">' + esc(tcName) + " · " + esc(tcPhone) + "</p>" +
        '<p style="font-size:11px;color:#9a9a9a"><a href="' + SITE + '/portal" style="color:#9a9a9a">Manage your preferences</a> · <a href="' + SITE + '/portal" style="color:#9a9a9a">Unsubscribe</a></p></div>';

      const ok = await sendResend({ to: agent.email!, subject, html, replyTo: (tc && tc.email) || undefined });
      if(!ok){ failed++; continue; }
      const nowIso = new Date().toISOString();
      for(const f of active){ await supabaseAdmin.from("files").update({ friday_summary_sent_at: nowIso }).eq("id", f.id); }
      sent++;
    } catch(e){ failed++; console.error("[friday-summary] agent failed", agent.id, e); }
  }
...
async function sendResend(args: { to: string; subject: string; html: string; replyTo?: string }): Promise<boolean> {
  ...
  for(let attempt=0; attempt<2; attempt++){
    const resp = await fetch("https://api.resend.com/emails", { method:"POST", headers:{ Authorization:"Bearer "+apiKey, "Content-Type":"application/json" }, body: JSON.stringify(body) });
